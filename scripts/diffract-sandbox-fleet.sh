...
# The deployment domain (written by setup.sh; empty for IP-only/no-domain mode).
# When set, per-sandbox Caddy sites are labelled "<domain>:<port>" so Caddy serves
# HTTPS on that port with the domain's cert; empty -> bare ":<port>" (HTTP only).
...
# One site block per allocated+running sandbox. Each owns its whole origin, so the
# SPA's root-relative /v1 and /assets resolve to THAT sandbox's web/gw ports.
regen_caddy() {
    local reg tmp changed domain site
    reg=$(reg_read)
    domain=""
    [ -f "$DOMAIN_FILE" ] && domain=$(tr -d '[:space:]' < "$DOMAIN_FILE")
    tmp=$(mktemp)
    echo "# AUTO-GENERATED by diffract-sandbox-fleet.sh — do not edit." > "$tmp"
    while IFS=$'\t' read -r name listen web gw; do
        [ -z "$name" ] && continue
        # With a domain, label "<domain>:<port>" so Caddy serves HTTPS (reusing the
        # domain cert) on the custom port; without one, bare ":<port>" (HTTP).
        if [ -n "$domain" ]; then site="$domain:$listen"; else site=":$listen"; fi
        cat >> "$tmp" <<CADDY
$site {
    handle /v1/* {
...
CADDY
    done < <(echo "$reg" | jq -r 'to_entries[] | select(.value.running == true) | [.key, .value.listen, .value.web, .value.gw] | @tsv')
