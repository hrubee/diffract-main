...
EOF
    ...
    # Install the multi-sandbox fleet reconciler — gives EVERY sandbox its own
    # origin (per-sandbox host port + Caddy site) so the operator can chat with
    # several sandboxes concurrently. Runs alongside (never replaces) the legacy
    # single-sandbox forwarder, so the proven default-sandbox path is untouched.
    if [ -f "$PROJECT_ROOT/scripts/diffract-sandbox-fleet.sh" ]; then
...
EOF
    ...
    # Multi-sandbox fleet reconciler service — per-sandbox origins for concurrent
    # chat. Independent of the legacy forwarder; if it fails it cannot take down
    # the default-sandbox chat (different ports, different Caddy snippet).
    cat <<EOF > /etc/systemd/system/diffract-sandbox-fleet.service
[Unit]
Description=Diffract Multi-Sandbox Fleet (per-sandbox concurrent chat)
...
Environment=PATH=$PATH
Environment=NEMOCLAW_AGENT=hermes
ExecStart=/usr/local/bin/diffract-sandbox-fleet.sh
Restart=always
RestartSec=10
...
EOF
    ...
    # Multi-sandbox fleet: seed an empty Caddy snippet (so the main Caddyfile's
    # `import` never fails before the reconciler has written anything), open the
    # per-sandbox listen-port range in ufw (no-op while ufw is inactive; persists),
    # then enable + start the reconciler.
    mkdir -p /var/lib/diffract /etc/caddy
    [ -f /etc/caddy/diffract-sandboxes.caddy ] || echo "# placeholder — generated by diffract-sandbox-fleet.sh" > /etc/caddy/diffract-sandboxes.caddy
    # Record the domain (empty in IP/no-domain mode) so the fleet labels per-sandbox
    # Caddy sites "<domain>:<port>" and Caddy serves HTTPS on the custom port.
    printf '%s' "${DOMAIN:-}" > /var/lib/diffract/domain
    if command -v ufw >/dev/null 2>&1; then
        ufw allow 8443:8446/tcp >/dev/null 2>&1 || true
    fi
    if [ -x /usr/local/bin/diffract-sandbox-fleet.sh ]; then
        systemctl enable diffract-sandbox-fleet.service
        systemctl restart diffract-sandbox-fleet.service
    fi
    ...
    # Pull in the per-sandbox site blocks (one `:<listen> { ... }` per sandbox,
    # generated by diffract-sandbox-fleet.sh) as top-level siblings of the main
    # site. The file is always present (seeded as a placeholder above), so this
    # import never breaks `caddy validate` even before any sandbox is published.
    if ! grep -q 'import /etc/caddy/diffract-sandboxes.caddy' /etc/caddy/Caddyfile; then
        printf '\nimport /etc/caddy/diffract-sandboxes.caddy\n' >> /etc/caddy/Caddyfile
    fi
