...
EOF
    ...
    # in-block `import` never fails before the reconciler has written anything),
    # then enable + start the reconciler. Per-sandbox chat is path-based on the
    # single existing origin (/<name>/agent/), so NO extra ports/ufw rules needed.
    ...
    # Record the domain (used by the fleet self-test to probe the public origin).
    ...
    # The per-sandbox PATH routes (handle /<name>/agent/*, generated by
    # diffract-sandbox-fleet.sh) are imported INSIDE the main site block above, so
    # they serve on the single existing origin (no extra ports/certs). The snippet
    # is always present (seeded as a placeholder), so the import never breaks
    # `caddy validate` even before any sandbox is published.
